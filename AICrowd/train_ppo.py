    ...
    def step(self, action):
        ...
        try:
            ...
            kpis = {
                ...
                "1-thermal_resilience": evaluation[
                    "one_minus_thermal_resilience_proportion"
                ]["value"],
                "normalized_unserved_energy": evaluation[
                    "power_outage_normalized_unserved_energy_total"
                ]["value"],
            }
            reward = self.reward_calc.score(kpis)
        except Exception as exc:  # pragma: no cover - fallback when evaluation fails
            reward = _reward
            info = info or {}
...
def main(total_timesteps=10000, model_path="ppo_aicrowd.zip"):
